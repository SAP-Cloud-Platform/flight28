...
FUNCTION /DMO/FLIGHT_TRAVEL_CALC_PRI_28.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IT_FLIGHT) TYPE  /DMO/IF_FLIGHT_LEGACY28=>TT_FLIGHT
*"       OPTIONAL
*"----------------------------------------------------------------------
  TYPES BEGIN OF ty_seats_occupied.
  INCLUDE TYPE /dmo/flight28.
  TYPES   distance       TYPE /dmo/connecti_28-distance.
  TYPES   distance_unit  TYPE /dmo/connecti_28-distance_unit.
  ...
  IF it_flight IS SUPPLIED.
    SELECT FROM /dmo/booking28 AS booking
      JOIN     /dmo/connecti_28 AS connection
                 ON  booking~carrier_id    = connection~carrier_id
                 AND booking~connection_id = connection~connection_id
      JOIN     /dmo/flight28 AS flight
